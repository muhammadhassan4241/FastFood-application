import { ArrowRight, Clock3, Flame, Leaf, Sparkles, Star, Truck, Utensils } from "lucide-react";
  ...
  return <div className="overflow-hidden">
    <section className="relative px-5 pb-20 pt-12 sm:px-8 md:px-10 md:pt-20">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="relative mx-auto w-full max-w-xl animate-fade-up [animation-delay:140ms]">
          <div className="absolute -right-2 top-8 z-10 rounded-2xl border border-white/50 bg-white/80 p-3 shadow-xl backdrop-blur-md dark:border-white/10 dark:bg-zinc-900/80 sm:-right-5"><div className="flex items-center gap-2"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-500/15 text-emerald-500"><Leaf size={18} /></div><div><p className="text-xs font-black">Made fresh</p><p className="text-[10px] text-zinc-500">Every single order</p></div></div></div>
          <div className="relative aspect-square overflow-hidden rounded-[2.5rem] bg-zinc-900 shadow-2xl shadow-orange-950/20"><div className="absolute inset-0 bg-gradient-to-br from-orange-500/30 via-transparent to-zinc-950/80" />{heroProduct && <img src={heroProduct.image} alt="Featured Food World dish" className="h-full w-full object-cover opacity-90 mix-blend-screen transition duration-700 hover:scale-105" />}<div className="absolute bottom-6 left-6 right-6 flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-300">Tonight's pick</p><p className="mt-1 text-2xl font-black text-white">{heroProduct?.title || heroProduct?.name || "Made with love"}</p></div><div className="rounded-2xl bg-white/15 px-3 py-2 text-right text-white backdrop-blur-md"><p className="text-[10px] uppercase tracking-wider text-white/60">Starting at</p><p className="font-black text-orange-300">Rs. {Number(heroProduct?.price || 699).toLocaleString()}</p></div></div></div>
          <div className="absolute -bottom-5 -left-3 rounded-2xl bg-zinc-950 px-4 py-3 text-white shadow-xl dark:bg-white dark:text-zinc-950"><p className="flex items-center gap-2 text-xs font-black"><Sparkles size={15} className="text-orange-400" />{cartCount ? `${cartCount} items in your cart` : "Your next craving awaits"}</p></div>
        </div>
      </div>
    </section>
